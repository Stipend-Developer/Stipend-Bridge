...
const controllerContractPath = path.join(
  __dirname,
  "stipend-token-smart-contracts",
  "./../contracts/controller/"
);
const factoryContractPath = path.join(
  __dirname,
  "stipend-token-smart-contracts",
  "./../contracts/factory/"
);
const tokenContractPath = path.join(
  __dirname,
  "stipend-token-smart-contracts",
  "./../contracts/token/"
);
const utilsContractPath = path.join(
  __dirname,
  "stipend-token-smart-contracts",
  "./../contracts/utils/"
);
...
const compilationInput = {
  "OwnableContract.sol": fs.readFileSync(
    utilsContractPath + "OwnableContract.sol",
    "ascii"
  ),
  "OwnableContractOwner.sol": fs.readFileSync(
    utilsContractPath + "OwnableContractOwner.sol",
    "ascii"
  ),
  "IndexedMapping.sol": fs.readFileSync(
    utilsContractPath + "IndexedMapping.sol",
    "ascii"
  ),
  "Controller.sol": fs.readFileSync(
    controllerContractPath + "Controller.sol",
    "ascii"
  ),
  "ControllerInterface.sol": fs.readFileSync(
    controllerContractPath + "ControllerInterface.sol",
    "ascii"
  ),
  ...
  "MembersInterface.sol": fs.readFileSync(
    factoryContractPath + "MembersInterface.sol",
    "ascii"
  ),
...
const mainnetUrls = ['https://data-seed-prebsc-2-s1.binance.org:8545/'];

const wspdAddress = "0x5ce2C91bED1ed63A52E709009115a5c7a03925BC";
...
async function check(mainnetUrl) {
  ...
  const solcOutput = await solc.compile(
  ...
  const wspdContract = await getContractAndCompareCode(
    "WSPD",
    wspdAddress,
    solcOutput
  );
  ...
  const controllerContract = await getContractAndCompareCode(
    "Controller",
    controllerAddress,
    solcOutput
  );
  ...
  const factoryContract = await getContractAndCompareCode(
    "Factory",
    factoryAddress,
    solcOutput
  );
  for (let i = 0; i < merchants.length; i++) {
    const merchantDepostAddress = await factoryContract.methods
      .setMerchantDepositAddress(merchants[i])
      .call();
    if (merchantDepostAddress == "")
      console.log(
        "warning: merchant",
        merchants[i],
      ...
      console.log(
        ...
        merchants[i],
        "deposit address",
        merchantDepostAddress
      );

    const custodianDepositAddress = await factoryContract.methods
      .setCustodianDepositAddress(merchants[i])
      .call();
    if (custodianDepositAddress == "")
      console.log(
        "warning: merchant",
        merchants[i],
        "custodian deposit address undefined!!!"
      );
    else {
      console.log(
        "merchant",
  ...
  if (spdTotalInventory >= wspdTotalSupply)
    console.log("SPD in custody >= WSPD total supply, ok");
...
async function getContractAndCompareCode(contractName, address, solcOutput) {
  const abi =
    solcOutput.contracts[contractName + ".sol:" + contractName].interface;
  const contract = await new web3.eth.Contract(JSON.parse(abi), address);
  const contractSolcCode =
    "0x" +
    solcOutput.contracts[contractName + ".sol:" + contractName].runtimeBytecode;
