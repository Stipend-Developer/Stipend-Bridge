...
contract('Controller', function(accounts) {
    ...
    beforeEach('create controller and transfer wspd ownership to it', async function () {
        wspd = await WSPD.new();
    ...
    describe('as owner', function () {
        ...
        it("should check transfer succeeds when not paused.", async function () {
            const balanceBefore = await wspd.balanceOf(admin)
            assert.equal(balanceBefore, 0);

            await controller.mint(admin, 100, {from: factory});
            const balanceAfterMint = await wspd.balanceOf(admin);
            assert.equal(balanceAfterMint, 100);

            await wspd.transfer(other, 20);
            const balanceAfterTransfer = await wspd.balanceOf(admin)
        ...
        it("should check transfer fails after pause.", async function () {
            ...
            await expectThrow(wspd.transfer(other, 20));
        });
        ...
        it("should check burn fails after pause.", async function () {
            ...
            await wspd.transfer(controller.address, 20, {from: factory})
            await controller.pause();
        ...
        it("should check transfer succeeds after unpause.", async function () {
            ...
            await controller.pause();
            const isPausedBefore = await wspd.paused.call();
            assert.equal(isPausedBefore, true);

            await expectThrow(wspd.transfer(other, 20));

            await controller.unpause();
            const isPausedAfter = await wspd.paused.call();
    ...
    describe('as factory', function () {
        it("mint", async function () {
            ...
            await controller.mint(admin, 100, {from: factory});
            const balanceAfter = await wspd.balanceOf(admin);
        ...
        it("mint of token that controller does not own should fail", async function () {
            await controller.callTransferOwnership(wspd.address, admin)
            await wspd.claimOwnership()
            await expectThrow(controller.mint(admin, 100, {from: factory}));
        });
        ...
        it("burn", async function () {
            ...
            const balanceBefore = await wspd.balanceOf(factory)
            assert.equal(balanceBefore, 100);
            ...
            const balanceAfter = await wspd.balanceOf(factory);
    ...
    describe('not as factory', function () {
        ...
        it("burn reverts.", async function () {
            ...
            const balanceBefore = await wspd.balanceOf(other)
            assert.equal(balanceBefore, 100);

            // when burning through factory we only need to approve.
            // here we transfer since checking internally.
            await wspd.transfer(controller.address, 20, {from: other})
    ...
    describe('as anyone', function () {
        ...
        it("check getToken.", async function () {
            ...
            assert.equal(gotToken, wspd.address);
        });
    });
});
