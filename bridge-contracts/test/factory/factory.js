...
const WSPD = artifacts.require("./token/WSPD.sol")
...
const REQUEST_SPD_DEPOSIT_ADDRESS_FIELD = 3
const REQUEST_SPD_TXID_FIELD            = 4
...
contract('Factory', function(accounts) {
    ...
    beforeEach('create contracts', async function () {
        wspd = await WSPD.new();
        controller = await Controller.new(wspd.address);
        ...
        await wspd.transferOwnership(controller.address)
        await controller.callClaimOwnership(wspd.address)

        await members.setCustodian(custodian0);
        await members.addMerchant(merchant0);
        await members.addMerchant(merchant1);
        await members.addMerchant(merchant2); // this merchant does not set spd deposit address
    ...
    describe('as merchant', function () {
        ...
        it("addMintRequest", async function () {
            const balanceBefore = await wspd.balanceOf(merchant0)
            ...
            assert.equal(request[REQUEST_SPD_DEPOSIT_ADDRESS_FIELD], custodianDepositAddressForMerchant0)
            assert.equal(request[REQUEST_SPD_TXID_FIELD], txid0)
            ...
            const balanceAfter = await wspd.balanceOf(merchant0)
            assert.equal(balanceAfter, 0);
        });

        it("addMintRequest with empty spdDepositAddress fails", async function () {
            await expectThrow(factory.addMintRequest(amount, txid0, "", {from}));
        });

        it("addMintRequest with non existing spdDepositAdress fails", async function () {
            await expectThrow(factory.addMintRequest(amount, txid0, otherAddress, {from}));
        });

        it("addMintRequest with other merchant's custodian spdDepositAdress fails", async function () {
        ...
        it("add several mint requests and see nonce is incremented", async function () {
            ...
            assert.equal(request[REQUEST_SPD_TXID_FIELD], txid2)
        });
        ...
        it("burn", async function () {
            ...
            await wspd.approve(factory.address, amount, {from});
        ...
        it("burn without setting merchant spdDepositAdress beforehand fails", async function () {
            const { logs } = await factory.addMintRequest(amount, txid0, custodianDepositAddressForMerchant2, {from: merchant2});
            await factory.confirmMintRequest(logs[0].args.requestHash, {from: custodian0});

            const balanceBefore = await wspd.balanceOf(merchant2)
            assert.equal(balanceBefore, amount);

            await wspd.approve(factory.address, amount, {from: merchant2});
            ...
            const balanceAfter = await wspd.balanceOf(merchant2)
            assert.equal(balanceAfter, 0);
        });
        ...
        it("burn without being the configured factory in controller fails", async function () {
            const { logs } = await factory.addMintRequest(amount, txid0, custodianDepositAddressForMerchant0, {from: merchant0});
            await factory.confirmMintRequest(logs[0].args.requestHash, {from: custodian0})
            await wspd.approve(factory.address, amount, {from: merchant0});
    ...
    describe('as anyone', function () {
        ...
        it("check create contract with 0 controller address fails", async function () {
